...
export default function ViewRequestorLeavePage({ leave, departmentRole, headName, vpName, employeeName, departmentName }: Props) {
    ...
    const isPending = leave.status === null;
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                    ...
                    <input type="date" value={formatDateForInput(leave.created_at)} disabled className="w-full p-2.5 rounded-lg border border-gray-200 dark:border-zinc-700 bg-gray-50 dark:bg-zinc-800/50 text-gray-700 dark:text-zinc-400 cursor-not-allowed outline-none" />
                </div>
                <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="space-y-2">
                    ...
                    <input type="date" value={formatDateForInput(leave.date)} disabled className="w-full p-2.5 rounded-lg border border-gray-200 dark:border-zinc-700 bg-gray-50 dark:bg-zinc-800/50 text-gray-700 dark:text-zinc-400 cursor-not-allowed outline-none" />
                </div>
                <div className="space-y-2">
                    <label className="text-sm font-semibold text-gray-900 dark:text-zinc-100">Time From</label>
                    <input type="time" value={formatTimeForInput(leave.time_from)} disabled className="w-full p-2.5 rounded-lg border border-gray-200 dark:border-zinc-700 bg-gray-50 dark:bg-zinc-800/50 text-gray-700 dark:text-zinc-400 cursor-not-allowed outline-none" />
                </div>
                <div className="space-y-2">
                    <label className="text-sm font-semibold text-gray-900 dark:text-zinc-100">Time To</label>
                    <input type="time" value={formatTimeForInput(leave.time_to)} disabled className="w-full p-2.5 rounded-lg border border-gray-200 dark:border-zinc-700 bg-gray-50 dark:bg-zinc-800/50 text-gray-700 dark:text-zinc-400 cursor-not-allowed outline-none" />
                </div>
            </div>
            ...
            <div className="pt-6 mt-6 border-t border-gray-200 dark:border-zinc-800">
                ...
                <div className="flex flex-col md:flex-row gap-4">
                    {/* 1. DEPARTMENT HEAD SECTION (Actionable) */}
                    <div className="flex-1 p-4 rounded-xl border border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-sm flex flex-col justify-between">
                        <div>
                        ...
                        <div className="mt-4">
                            {leave.status === 0 ? (
                                ...
                                <div className="flex gap-2">
                                    <button disabled={isUpdating} onClick={() => handleStatusUpdate(1)} className="flex-1 px-2 py-1.5 rounded-lg text-[10px] font-bold bg-gray-900 text-white dark:bg-zinc-100 dark:text-zinc-900 hover:opacity-90 transition-all disabled:opacity-50">
                                        Approve
                                    </button>
                                    <button disabled={isUpdating} onClick={() => handleStatusUpdate(0)} className="flex-1 px-2 py-1.5 rounded-lg text-[10px] font-bold bg-white text-red-600 border border-red-200 dark:bg-zinc-900 dark:border-red-900/30 hover:bg-red-50 dark:hover:bg-red-950/30 transition-all disabled:opacity-50">
                                        Reject
                                    </button>
                                </div>
                            ) : (
                    ...
                    {/* 2. VP ADMIN SECTION (Read-Only Status) */}
                    <div className="flex-1 p-4 rounded-xl border border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-sm flex flex-col justify-between">
                        <div>
                            <p className="text-[10px] text-gray-500 font-bold mb-1 uppercase">Approved by:</p>
                            <p className="text-[10px] text-gray-500 font-bold mb-1 uppercase">Vice President for Administration</p>
                        ...
                        <div className="mt-4">
                            {leave.status === 3 ? (
                                ...
                                <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-gray-100 text-gray-500 border border-gray-200 dark:bg-zinc-800 dark:text-zinc-400 dark:border-zinc-700">Stopped at Head Level</span>
                            ) : leave.status === 1 ? (
                                <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-blue-50 text-blue-600 border border-blue-200 dark:bg-blue-500/10 dark:text-blue-400 dark:border-blue-500/20">Pending VP Approval</span>
                            ) : (
                                <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-gray-100 text-gray-500 border border-gray-200 dark:bg-zinc-800 dark:text-zinc-400 dark:border-zinc-700">Waiting for Head</span>
                            )}
                        </div>
                    </div>
                </div>
