...
export default function LeaveManagementPage({ leavesList, defaultQuery = "" }: Props) {
    ...
    const calculateDays = (from: Date | string | null, to: Date | string | null) => {
        if (!from || !to) return "—";
    ...
    return (
        <div className="space-y-6 flex flex-col w-full">
            ...
            <div className="bg-white dark:bg-zinc-900 border-x border-t border-gray-200 dark:border-zinc-800 rounded-t-xl shadow-sm overflow-hidden flex flex-col w-full">
                                    ...
                                    <div key={leave.id} className="p-4 hover:bg-gray-50 dark:hover:bg-zinc-800/30 transition-colors space-y-3 cursor-pointer">
                                        <div className="flex justify-between items-start gap-4">
                                            <div className="flex flex-col gap-1.5">
                                                ...
                                                <span className="text-xs text-gray-400 dark:text-zinc-500 font-mono">Reason: {leave.reason}</span>
                                            </div>
                                        ...
                                        <div className="grid grid-cols-2 gap-y-3 gap-x-4 text-sm pt-2 border-t border-gray-100 dark:border-zinc-800/80 transition-colors">
                                            <div>
                                                <span className="text-xs font-semibold opacity-70 text-gray-500 dark:text-zinc-400 uppercase tracking-wider block mb-0.5">Duration</span>
                                                <span className="font-medium text-gray-900 dark:text-zinc-100">{formatDate(leave.date_from)}</span>
                                                <span className="block text-xs text-gray-500 dark:text-zinc-400">to {formatDate(leave.date_to)}</span>
                                            </div>
                                            <div>
                                                ...
                                                <span className="font-medium text-gray-900 dark:text-zinc-100">{calculateDays(leave.date_from, leave.date_to)} Day(s)</span>
                        ...
                        <table className="w-full text-left border-collapse whitespace-nowrap">
                            <thead className="bg-gray-50 dark:bg-zinc-800/50 border-b border-gray-200 dark:border-zinc-800 transition-colors duration-300">
                                <tr>
                                    ...
                                    <th scope="col" className="px-6 py-3 text-xs font-bold text-gray-500 dark:text-zinc-400 uppercase tracking-wider">Duration</th>
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-zinc-800/80 bg-white dark:bg-zinc-900 transition-colors duration-300">
                                {leavesList.length > 0 ? (
                                    leavesList.map((leave) => {
                                        const status = getStatusDetails(leave.status);
                                        return (
                                            <tr key={leave.id} className="hover:bg-gray-50 dark:hover:bg-zinc-800/30 transition-colors cursor-pointer">
                                                <td className="px-6 py-4">
                                                    <div className="text-sm font-bold text-gray-900 dark:text-zinc-100 capitalize">{leave.leave_type}</div>
                                                    <div className="text-xs text-gray-500 dark:text-zinc-400 mt-0.5 font-mono">Reason: {leave.reason}</div>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <div className="text-sm text-gray-700 dark:text-zinc-300 font-medium">
                                                        {formatDate(leave.date_from)} <span className="text-gray-400 mx-1">→</span> {formatDate(leave.date_to)}
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <div className="text-sm font-bold text-gray-900 dark:text-zinc-100">
                                                        {calculateDays(leave.date_from, leave.date_to)}
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4">
