...
export function Sidebar({ userRole, userId, profilePicture, surname}: SidebarProps) {
            ...
            <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-200 transform transition-transform duration-200 ease-in-out ${isOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 md:sticky md:top-0 md:h-screen md:min-w-64 flex flex-col`}>
                ...
                <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
                    {navItems.map((item) => {
                        ...
                        return (
                            <Link key={item.name} href={item.href || "#"} className={`flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-xl transition-colors duration-200 ${isActive ? "bg-green-50 text-[#1a6b36] border border-green-100" : "text-gray-600 hover:bg-slate-50 hover:text-gray-900"}`}>
                ...
                {userRole === "admin" && (
                    <div className="px-4 py-2 border-t border-gray-50">
                        <Link
                            ...
                            className={`flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-xl transition-colors duration-200 ${isAdminActive
                                ? "bg-green-50 text-[#1a6b36] border border-green-100"
                                : "text-gray-600 hover:bg-slate-50 hover:text-gray-900"}`}
                        >
                            <svg className={`w-5 h-5 ${isAdminActive ? "text-[#1a6b36]" : "text-gray-400"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="p-4 border-t border-gray-100">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                            <span className="text-xs font-bold text-gray-700 uppercase">{surname}</span>
