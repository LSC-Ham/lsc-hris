...
export default function EmployeesTable({
    ...
    // Helper function to handle the badge colors cleanly
    const getRemarksClasses = (remarks?: string | null) => {
        ...
        // Probationary, null, or any other unhandled status defaults to yellow
                ...
                <div className="block md:hidden divide-y divide-gray-100 dark:divide-zinc-800/80 transition-colors duration-300">
                    {employeesList.length > 0 ? (
                        employeesList.map((emp: any) => {
                            const empRemarks = emp.remarks || emp.employees?.remarks;
                            return (
                                <div
                                    key={emp.id}
                                    ...
                                    <div className="flex justify-between items-start gap-4">
                                        <div className="flex flex-col gap-2.5">
                                            ...
                                            <div className="flex flex-wrap items-center gap-2 mt-0.5">
                                                ...
                                                <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-bold border ${getRemarksClasses(empRemarks)}`}>
                                                    {empRemarks || "No Status"}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
