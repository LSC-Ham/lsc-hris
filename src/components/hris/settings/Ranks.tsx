// src/components/hris/settings/Ranks.tsx
"use client";

import { useState, useRef } from "react";
import { ConfirmModal } from "@/components/ui/ConfirmModal";
import { toast } from "sonner";
import { addRank, deleteRank, updateRank } from "@/actions/admin/settings/ranks/action";
...
export default function Ranks({ data }: RanksTemplateProps) {
    ...
    const handleDeleteClick = (id: string, name: string) => {
        setPendingAction({ type: "delete", id, rankName: name });
        setIsModalOpen(true);
    };
    ...
    const getModalConfig = () => {
        switch (pendingAction?.type) {
            case "add":
                return {
                    title: "Add Rank",
                    message: "Are you sure you want to add this new rank?",
                    confirmText: "Add Rank",
            ...
            case "delete":
                return {
                    ...
                    message: (
                        <>
                            Are you sure you want to permanently delete the <strong>{pendingAction.rankName}</strong> rank? This action cannot be undone.
                        </>
                    ),
                    confirmText: "Delete",
    ...
    return (
        <div className="space-y-6">
            <form ref={addFormRef} action={handleAddClick} className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-3 bg-gray-50 dark:bg-zinc-950/50 p-4 rounded-lg border border-gray-200 dark:border-zinc-800">
                <input
                    type="text"
                    name="rank"
                ...
                <table className="w-full text-left text-sm">
                    <thead className="bg-gray-50 dark:bg-zinc-950/50 border-b border-gray-200 dark:border-zinc-800">
                        <tr>
                            ...
                            <th className="px-4 py-3 font-medium text-gray-700 dark:text-zinc-300 w-1/2">Order</th>
                            ...
                            <tr key={r.id} className="hover:bg-gray-50/50 dark:hover:bg-zinc-800/30 transition-colors">
                                {editingId === r.id ? (
                                    <td colSpan={3} className="px-4 py-3">
                                        <form
                                            action={(formData) => handleUpdateClick(r.id, formData)}
                                    ...
                                    <>
                                        <td className="px-4 py-3 font-medium text-gray-900 dark:text-zinc-100">{r.rank}</td>
                                        <td className="px-4 py-3 text-gray-500 dark:text-zinc-400">{r.description || "—"}</td>
                                        <td className="px-4 py-3 text-gray-500 dark:text-zinc-400">{r.order || "—"}</td>
                                        <td className="px-4 py-3 text-right space-x-3 whitespace-nowrap">
                                            ...
                                            <button
                                                type="button"
                                                onClick={() => handleDeleteClick(r.id, r.rank)}
